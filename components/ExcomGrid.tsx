...
export default function ExcomGrid() {
    ...
    return (
        <section id="members" className="py-24 px-4 bg-brand-cream min-h-screen relative overflow-visible z-10">
                    ...
                    {excomMembers.map((member, i) => (
                        ...
                        >
                            <div className="relative aspect-[4/5]">
                                {/* Shadow Box */}
                                <div className="absolute inset-0 bg-brand-dark translate-x-3 translate-y-3 rounded-xl" />

                                {/* Main Card */}
                                <div className="absolute inset-0 bg-white border-2 border-brand-dark rounded-xl overflow-hidden hover:-translate-y-2 hover:-translate-x-2 transition-transform duration-200 ease-out">
                                    {/* Image */}
                                    <div className="h-[80%] w-full relative border-b-2 border-brand-dark">
                                        {member.image ? (
                                            <img
                                                src={member.image}
                                                alt={member.name}
                                        ...
                                        <div className="absolute top-2 right-2 bg-brand-yellow text-brand-dark text-[10px] font-bold uppercase px-2 py-1 border border-brand-dark rounded-md">
                                            {member.role}
                                        </div>
                                    </div>

                                    {/* Name */}
                                    <div className="h-[20%] flex items-center justify-center p-2 bg-white">
                                        <h3 className="font-display text-xl uppercase leading-none text-center text-brand-dark">{member.name}</h3>
                                    </div>
                                </div>
                            </div>
                        </motion.div>
            ...
            {/* Wave Transition to Memories (Yellow) */}
